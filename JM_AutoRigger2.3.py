...
def surfaceCtrlConnection(*args):
...
my_window = cmds.window("myWindow", title="JM_Facial_AutoRigger2.3")
...
def importCOGs(*args):
    ...
    rePath = os.environ["HOME"] +'/maya/scripts'
    rePathCOG = rePath + '/exportEyeCtrl.mb'
    
    cmds.file( '%s' % rePathCOG, i=True, defaultNamespace = True ) 


def eyeCtrlCnst(*args):
    
    import maya.cmds as cmds
    # Clear any existing selections
    cmds.select(cl=True)
    
    # Create Point Constraints for each pair of selected objects
    point_constraints = [
        ("R_EYE_CORNER_IN_CTRL", "R_eye7_JNT"),
        ("R_EYE_UPLID2_CTRL", "R_eye2_JNT"),
        ("R_EYE_UPLID1_CTRL", "R_eye1_JNT"),
        ("R_EYE_UPLID3_CTRL", "R_eye3_JNT"),
        ("R_EYE_CORNER_OUT_CTRL", "R_eye8_JNT"),
        ("R_EYE_DOWNLID3_CTRL", "R_eye6_JNT"),
        ("R_EYE_DOWNLID1_CTRL", "R_eye4_JNT"),
        ("R_EYE_DOWNLID2_CTRL", "R_eye5_JNT"),
        ("L_EYE_CORNER_IN_CTRL", "L_eye7_JNT"),
        ("L_EYE_UPLID2_CTRL", "L_eye2_JNT"),
        ("L_EYE_UPLID1_CTRL", "L_eye1_JNT"),
        ("L_EYE_UPLID3_CTRL", "L_eye3_JNT"),
        ("L_EYE_CORNER_OUT_CTRL", "L_eye8_JNT"),
        ("L_EYE_DOWNLID3_CTRL", "L_eye6_JNT"),
        ("L_EYE_DOWNLID1_CTRL", "L_eye4_JNT"),
        ("L_EYE_DOWNLID2_CTRL", "L_eye5_JNT")
    ]
    
    for source, target in point_constraints:
...
pm.button(l= "Eye CTRL setup", command=importCOGs)

pm.text(label="complete the Blink setup and make it symmetri")
pm.button(l= "Finish EyeCtrl constraints setting", command=eyeCtrlCnst)
...
# 생성 버튼 추가
create_button = pm.button(label="Create Locators", command=makePri)
create_button = pm.button(label="Connect Motionpath", command=makePri2)
pm.text(label="add influence the facial joints to the face")
create_button = pm.button(label="Install NURBS Ctrls", command=addSurfaceSetting)
create_button = pm.button(label="Connect NURBS Ctrls", command=surfaceCtrlConnection)
